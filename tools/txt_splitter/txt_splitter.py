...
def write_part(output_dir: Path, stem: str, suffix: str, index: int, content: str) -> Path:
    output_path = output_dir / f"{stem}_part_{index:03d}{suffix}"
...
def main():
    parser = argparse.ArgumentParser(description="Split a TXT or Markdown file into smaller files")
    parser.add_argument("--input_file", required=True, help="TXT or Markdown file to split")
    ...
    if input_file.suffix.lower() not in {".txt", ".md"}:
        print("[ERROR] Input file must be a .txt or .md file.")
        sys.exit(1)
    ...
    for index, part in enumerate(parts, start=1):
        try:
            output_path = write_part(output_dir, input_file.stem, input_file.suffix, index, part)
